...
import { invertExplodedModelState } from '../reducers/infoBar.reducer';
 ...
 * @propert {Function} {buttonAction} - runs an action when button ins clicked
 */
...
export default function InfoBar() {
  /**
   * @todo do not return the whole state error due loging the state to check
   */
  const storeSelector = useSelector((state) => state);
  console.log('store state of the exploded model', storeSelector.explodedModel);

  /**
   * Update collapsed model using Store
   */
  const dispatch = useDispatch();
  function explodeCollapseAction() {
    dispatch(invertExplodedModelState());
  }
  return (
    <>
      <div className="fixed left-0 top-0 z-10 flex h-full flex-col content-center items-center justify-start gap-2 pt-6">
        ...
        <InfoBarControlButton
          ...
          buttonAction={explodeCollapseAction}
        />
        <InfoBarControlButton
          path={
            <path
